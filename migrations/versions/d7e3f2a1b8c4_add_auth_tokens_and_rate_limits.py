...
from util import index_exists, table_exists
...
def downgrade():
    if index_exists("rate_limit_buckets", "ix_rate_limit_buckets_bucket_key"):
        op.drop_index("ix_rate_limit_buckets_bucket_key", table_name="rate_limit_buckets")
    if table_exists("rate_limit_buckets"):
        op.drop_table("rate_limit_buckets")
    if index_exists("auth_tokens", "ix_auth_tokens_expires_at"):
        op.drop_index("ix_auth_tokens_expires_at", table_name="auth_tokens")
    if index_exists("auth_tokens", "ix_auth_tokens_user_id"):
