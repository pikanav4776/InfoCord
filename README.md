...
| Client handles conflict resolution | **Partial** | UI shows toast and refreshes notes on 409; no manual merge UI or “keep both” flow yet |
...
├── crypto_auth_service.dart  # Flutter crypto/auth scaffold (post-MVP)
...
| **5 — Version sync** | `version` field; server validates on update; client conflict handling | **Partial** | 409 conflicts detected; no merge UI, retry queue, or “keep both” flow |
...
**MVP verdict:** **Substantially complete.** The only MVP gap called out in this README is **Phase 5 conflict-resolution UX** (marked Partial above). Everything else in Phases 0–7 is implemented.
...
- [ ] Build the Flutter mobile app (currently scaffold-only in `crypto_auth_service.dart`)
- [ ] Align mobile PBKDF2 iterations with web (**web: 310,000**; Dart scaffold currently says **600,000** — must match before cross-platform use)
- [ ] App icons, splash screens, store screenshots, and listing copy
- [ ] Apple Developer Program + Google Play Console accounts
- [ ] Privacy policy URL (required by both stores — even for US-only launch)
- [ ] Terms of service and account-deletion flow (Apple requires in-app account deletion for apps with accounts)
...
**Remaining steps**
- [ ] Publish a privacy policy covering: what is collected (email, name), what is E2EE (note content), cookies/sessions, third parties (Render, Neon), retention, and deletion
- [ ] Implement account + data deletion endpoint (required for App Store)
...
|----------|----------|--------------|
| **P0 — Must have** | Mobile product | No Flutter app in repo; only `crypto_auth_service.dart` scaffold |
| **P0 — Must have** | Store requirements | Privacy policy URL, terms of service, account deletion, app icons/screenshots |
| **P0 — Must have** | Production stability | In-memory tokens/rate limits; fix before scaling Render workers |
...
| **P2 — Nice to have** | MVP polish | Phase 5 conflict merge UI; offline/mobile sync; encrypted search |
| **P2 — Nice to have** | Business | Monetization model, formal product brief |

**Bottom line:** The **web MVP objectives in this README are met** (except Phase 5 conflict UX). The project is **not yet ready for App Store submission** — there is no shipping mobile app, no store-mandatory legal pages, and several production/ops gaps. Your checklist is largely accurate, with these corrections: **tests already exist (100 in `test_auth.py`)**, **architecture is stable but monolithic**, and **US/India compliance is broader than “skip GDPR.”**
...
- Privacy-preserving redesign notes: `documentation/new 29.txt`
